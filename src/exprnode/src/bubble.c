...
/* ~(a | b | ...) = ~a & ~b & ... */
static struct BoolExpr *_inv_or(struct BoolExpr *op) {
...
static struct BoolExpr *_inv_and(struct BoolExpr *op) {
    size_t length = op->data.xs->length;
    struct BoolExpr **xs;
    struct BoolExpr *temp;
    struct BoolExpr *y;
...
static struct BoolExpr *_inv_ite(struct BoolExpr *op) {
...
struct BoolExpr *BX_PushDownNot(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return BX_IncRef(ex);
