...
/* simple.c */
static struct BoolExpr *_simple_op(BX_Kind kind, size_t n,
                                   struct BoolExpr **xs);
static struct BoolExpr *_simple_op2(BX_Kind kind, struct BoolExpr *x0,
                                    struct BoolExpr *x1);
static struct BoolExpr *_simple_nop(BX_Kind kind, size_t n,
                                    struct BoolExpr **xs);

/* NOTE: Equality testing can get expensive, so keep it simple */
static bool _eq(struct BoolExpr *a, struct BoolExpr *b) { return (a == b); }
...
static size_t _count_orand_args(struct BoolExpr *op) {
    size_t count = 0;
...
static struct BoolExpr *_orand_simplify(struct BoolExpr *op) {
...
static size_t _count_xor_args(struct BoolExpr *op) {
    size_t count = 0;
...
/* NOTE: assume operator arguments are already simple */
static struct BoolExpr *_xor_simplify(struct BoolExpr *op) {
    bool parity = true;
    ...
    for (size_t i = 0; i < op->data.xs->length; ++i) {
        ...
        /* Xor(Xnor(x0, x1), x2) <=> Xnor(x0, x1, x2) */
            ...
            parity ^= true;
            for (size_t j = 0; j < xi->data.xs->items[0]->data.xs->length;
                 ++j) {
...
static struct BoolExpr *_eq_simplify(struct BoolExpr *op) {
...
/* NOTE: assumes arguments are already simple */
static struct BoolExpr *_not_simplify(struct BoolExpr *op) {
    return BX_Not(op->data.xs->items[0]);
}

/* NOTE: assumes arguments are already simple */
static struct BoolExpr *_impl_simplify(struct BoolExpr *op) {
...
/* NOTE: assumes arguments are already simple */
static struct BoolExpr *_ite_simplify(struct BoolExpr *op) {
...
struct BoolExpr *BX_Simplify(struct BoolExpr *ex) {
    struct BoolExpr *y;
