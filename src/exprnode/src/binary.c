...
static struct BoolExpr *_commutative_binify(struct BoolExpr *op) {
...
static struct BoolExpr *_eq_binify(struct BoolExpr *op) {
    if (op->data.xs->length == 2)
        return BX_IncRef(op);
    ...
    for (size_t i = 0, index = 0; i < (op->data.xs->length - 1); ++i) {
        for (size_t j = i + 1; j < op->data.xs->length; ++j, ++index) {
            CHECK_NULL_N(
                xs[index],
                BX_EqualN(2, op->data.xs->items[i], op->data.xs->items[j]),
                index, xs);
        }
    }
...
static struct BoolExpr *_fixed_binify(struct BoolExpr *op) {
    return BX_IncRef(op);
}
...
struct BoolExpr *BX_ToBinary(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return BX_IncRef(ex);
