...
void BX_Iter_Del(struct BX_Iter *it) { free(it); }

bool BX_Iter_Next(struct BX_Iter *it) {
    if (it->done)
        return true;
...
struct BoolExpr BX_Illogical = {
    1, BX_ILLOGICAL, BX_NNF | BX_SIMPLE, {.pcval = 0}};

struct BoolExpr *_bx_identity[16] = {
    NULL,     NULL,    NULL,     NULL, NULL, NULL, NULL, NULL,
    &BX_Zero, &BX_One, &BX_Zero, NULL, NULL, NULL, NULL, NULL,
};

struct BoolExpr *_bx_dominator[16] = {
    NULL,    NULL,     NULL, NULL, NULL, NULL, NULL, NULL,
    &BX_One, &BX_Zero, NULL, NULL, NULL, NULL, NULL, NULL,
};

struct BoolExpr *_lit_new(struct BX_Vector *lits, long uniqid) {
    struct BoolExpr *lit;
...
struct BoolExpr *_bx_orandxor_new(BX_Kind kind, size_t n,
                                  struct BoolExpr **xs) {
    if (n == 0)
...
struct BoolExpr *_eq_new(size_t n, struct BoolExpr **xs) {
    if (n <= 1)
...
static void _op_del(struct BoolExpr *op) {
...
**  x : { 1,  2,  3,  4, ...} => {1, 3, 5, 7, ...}
*/
static size_t _uniqid2index(long uniqid) {
    assert(uniqid != 0);
...
struct BoolExpr *BX_Literal(struct BX_Vector *lits, long uniqid) {
    ...
    lit =
        (index >= lits->length) ? (struct BoolExpr *)NULL : lits->items[index];
...
static struct BoolExpr *(*_boolexpr_inv[16])(struct BoolExpr *ex) = {
    _zero_inv, _one_inv, _log_inv, _ill_inv,

    _lit_inv,  _lit_inv, NULL,     NULL,

    _op_inv,   _op_inv,  _op_inv,  _op_inv,

    _not_inv,  _op_inv,  _op_inv,  NULL,
};
...
struct BoolExpr *BX_ITE(struct BoolExpr *s, struct BoolExpr *d1,
                        struct BoolExpr *d0) {
...
struct BoolExpr *BX_IncRef(struct BoolExpr *ex) {
...
static void (*_boolexpr_del[16])(struct BoolExpr *ex) = {
    ...
    _lit_del, _lit_del, NULL,    NULL,

    _op_del,  _op_del,  _op_del, _op_del,

    _op_del,  _op_del,  _op_del, NULL,
};

void BX_DecRef(struct BoolExpr *ex) {
    /* Input must not be NULL */
    assert(ex != NULL);
...
struct BX_Set *BX_Support(struct BoolExpr *ex) {
    struct BX_Set *s;
    struct BX_Iter *it;
...
unsigned long BX_Depth(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return 0;
...
unsigned long BX_Size(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return 1;
...
unsigned long BX_AtomCount(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return 1;
...
unsigned long BX_OpCount(struct BoolExpr *ex) {
    if (BX_IS_ATOM(ex))
        return 0;
...
bool BX_IsDNF(struct BoolExpr *ex) {
...
bool BX_IsCNF(struct BoolExpr *ex) {
