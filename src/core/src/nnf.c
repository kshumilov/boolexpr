...
static struct BoolExpr *_orandnot_nnfify(struct BoolExpr *op) {
    return BX_IncRef(op);
}

/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_xor_nnfify_conj(struct BoolExpr *op) {
    struct BoolExpr *y;
    ...
    if (op->data.xs->length == 2) {
        ...
        BX_DecRef(xn0);
        BX_DecRef(xn1);
        CHECK_NULL_1(or_x0_x1,
                     BX_OrN(2, op->data.xs->items[0], op->data.xs->items[1]),
                     or_xn0_xn1);
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_xor_nnfify_disj(struct BoolExpr *op) {
    struct BoolExpr *y;
    ...
    if (op->data.xs->length == 2) {
        ...
        CHECK_NULL_2(and_x0_xn1, BX_AndN(2, op->data.xs->items[0], xn1), xn1,
                     and_xn0_x1);
        BX_DecRef(xn1);
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_eq_nnfify(struct BoolExpr *op) {
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_impl_nnfify(struct BoolExpr *op) {
    struct BoolExpr *p, *q;
    struct BoolExpr *pn;
    struct BoolExpr *y;
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_ite_nnfify_conj(struct BoolExpr *op) {
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_ite_nnfify_disj(struct BoolExpr *op) {
    struct BoolExpr *s, *d1, *d0;
    struct BoolExpr *sn;
...
/* NOTE: assume operator arguments are already NNF */
static struct BoolExpr *_ite_nnfify(struct BoolExpr *op) {
    unsigned int num_ors = 0;
    unsigned int num_ands = 0;
...
struct BoolExpr *_nnfify(struct BoolExpr *ex) {
    if (BX_IS_NNF(ex))
        return BX_IncRef(ex);
...
struct BoolExpr *BX_ToNNF(struct BoolExpr *ex) {
    struct BoolExpr *nnf;
