...
static size_t _hash(struct BX_Set *set, struct BoolExpr *key) {
    return (size_t)key % _primes[set->_pridx];
}

static bool _eq(struct BoolExpr *key1, struct BoolExpr *key2) {
    return key1 == key2;
}

static void _list_del(struct BX_SetItem *list) {
    if (list) {
        _list_del(list->tail);
        BX_DecRef(list->key);
        free(list);
...
static bool _list_contains(struct BX_SetItem *list, struct BoolExpr *key) {
    if (!list)
        return false;
...
void BX_Set_Del(struct BX_Set *set) {
...
void BX_SetIter_Init(struct BX_SetIter *it, struct BX_Set *set) {
    it->_set = set;
...
void BX_SetIter_Next(struct BX_SetIter *it) {
    if (it->done)
        return;
    ...
    for (it->_index += 1; it->_index < _primes[it->_set->_pridx];
         it->_index += 1) {
...
static bool _set_insert(struct BX_Set *set, struct BoolExpr *key) {
...
static bool _enlarge(struct BX_Set *set) {
    struct BX_SetItem *item;
...
bool BX_Set_Insert(struct BX_Set *set, struct BoolExpr *key) {
    double load;
...
bool BX_Set_Remove(struct BX_Set *set, struct BoolExpr *key) {
    size_t index = _hash(set, key);
    struct BX_SetItem *item;
    struct BX_SetItem **tail;
...
bool BX_Set_Contains(struct BX_Set *set, struct BoolExpr *key) {
    size_t index = _hash(set, key);

    return _list_contains(set->items[index], key);
}

bool BX_Set_EQ(struct BX_Set *self, struct BX_Set *other) {
...
bool BX_Set_NE(struct BX_Set *self, struct BX_Set *other) {
    return !BX_Set_EQ(self, other);
}

bool BX_Set_LTE(struct BX_Set *self, struct BX_Set *other) {
    if (self->length > other->length)
        return false;
...
bool BX_Set_GT(struct BX_Set *self, struct BX_Set *other) {
    if (self->length <= other->length)
        return false;
...
bool BX_Set_GTE(struct BX_Set *self, struct BX_Set *other) {
    if (self->length < other->length)
        return false;
...
bool BX_Set_LT(struct BX_Set *self, struct BX_Set *other) {
    if (self->length >= other->length)
        return false;
...
bool BX_Set_Update(struct BX_Set *self, struct BX_Set *other) {
    struct BX_SetItem *item;
...
void BX_Set_Clear(struct BX_Set *set) {
    for (size_t i = 0; i < _primes[set->_pridx]; ++i) {
        if (set->items[i]) {
            _list_del(set->items[i]);
            set->items[i] = (struct BX_SetItem *)NULL;
...
struct BoolExpr **BX_Set_ToExprs(struct BX_Set *set) {
