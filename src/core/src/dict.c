...
static size_t _hash(struct BX_Dict *dict, struct BoolExpr *key) {
...
static void _list_del(struct BX_DictItem *list) {
    if (list) {
        _list_del(list->tail);
        BX_DecRef(list->key);
        BX_DecRef(list->val);
...
static struct BoolExpr *_list_search(struct BX_DictItem *list,
                                     struct BoolExpr *key) {
    if (!list)
...
void BX_Dict_Del(struct BX_Dict *dict) {
...
void BX_DictIter_Init(struct BX_DictIter *it, struct BX_Dict *dict) {
    it->_dict = dict;
...
void BX_DictIter_Next(struct BX_DictIter *it) {
    if (it->done)
        return;
    ...
    for (it->_index += 1; it->_index < _primes[it->_dict->_pridx];
         it->_index += 1) {
...
static bool _dict_insert(struct BX_Dict *dict, struct BoolExpr *key,
                         struct BoolExpr *val) {
...
static bool _enlarge(struct BX_Dict *dict) {
    struct BX_DictItem *item;
...
bool BX_Dict_Insert(struct BX_Dict *dict, struct BoolExpr *key,
                    struct BoolExpr *val) {
    double load;
...
bool BX_Dict_Remove(struct BX_Dict *dict, struct BoolExpr *key) {
    size_t index = _hash(dict, key);
    struct BX_DictItem *item;
    struct BX_DictItem **tail;
...
struct BoolExpr *BX_Dict_Search(struct BX_Dict *dict, struct BoolExpr *key) {
    size_t index = _hash(dict, key);

    return _list_search(dict->items[index], key);
}

bool BX_Dict_Contains(struct BX_Dict *dict, struct BoolExpr *key) {
    size_t index = _hash(dict, key);

    return _list_search(dict->items[index], key) != (struct BoolExpr *)NULL;
}

void BX_Dict_Clear(struct BX_Dict *dict) {
    for (size_t i = 0; i < _primes[dict->_pridx]; ++i) {
        if (dict->items[i]) {
            _list_del(dict->items[i]);
            dict->items[i] = (struct BX_DictItem *)NULL;
...
bool BX_Dict_Equal(struct BX_Dict *self, struct BX_Dict *other) {
...
bool BX_Dict_Update(struct BX_Dict *self, struct BX_Dict *other) {
    struct BX_DictItem *item;
